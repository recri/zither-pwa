...
[ ] PWA can exit with window.close().
[ ] PWA can be forced to update by changes to webmanifest or service worker.
[ ] Can solve button accessibility problems with sl-button, and use the
    sl-icon library, too.  
